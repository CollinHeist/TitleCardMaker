...
class NotificationGroup:
    ...
    def format_timedelta(td: timedelta, /) -> str:
        ...
        times = []
        if days > 1:
            times.append(f'{days} day' + ('s' if days > 1 else ''))
        if hours > 1:
            times.append(f'{hours} hour' + ('s' if hours > 1 else ''))
        if minutes > 1:
            times.append(f'{minutes} minute' + ('s' if minutes > 1 else ''))
        if seconds:
            times.append(f'{seconds} second' + ('s' if seconds > 1 else ''))
